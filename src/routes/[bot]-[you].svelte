            ...
            <div class="relative w-full p-6 overflow-y-auto h-full">
              <ul class="space-y-2">
                ...
                <li class="flex justify-{item.class}">
                  <div on:mousedown={() => { timer = Date.now() }} on:mousedown={handleSelect} on:touchstart={() => { timer = Date.now() }} on:touchend={handleSelect} on:pointerdown={() => { timer = Date.now() }} on:pointerup={handleSelect} class="{item.bg} relative max-w-xl px-4 py-2 text-gray-700 rounded shadow" >
